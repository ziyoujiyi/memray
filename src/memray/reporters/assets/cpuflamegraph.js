import {
  ...
  filterUninteresting,
  humanFileSize,
  makeCpuTooltipString,
  sumCpuSamples,
  resizeCpuGraph,
} from "./common";
window.resizeCpuGraph = resizeCpuGraph;
...
function onFilterThread() {
  ...
  if (thread_id === "-0x1") {
    ...
    filteredChart.registerFilter(FILTER_THREAD, (data) => {
      ...
      const totalCpuSamples = sumCpuSamples(filteredData.children);
      _.defaults(totalCpuSamples, filteredData);
      filteredData.n_cpu_samples = totalCpuSamples.n_cpu_samples;
      filteredData.value = totalCpuSamples.value;
      return filteredData;
    });
  }
...
function getTooltip() {
  let tip = d3
    ...
    .html((d) => {
      // const totalCount = humanFileSize(d.data.value);
      return makeCpuTooltipString(d.data, total_cpu_samples, merge_threads);
    })
    .direction((d) => {
...
function cpuColorMapper(d, originalColor) {
  // Highlighted nodes
  if (d.highlight) {
    return "orange";
  }
  // "builtin" / nodes that we don't want to highlight
  if (!d.data.name || !d.data.location) {    // TODO : trap
    return "#EEE";
  }
...
// https://github.com/spiermar/d3-flame-graph/blob/ba6b2d0f226720e717073f67f58ebeffc051d32c/README.md
function drawChart(chart_data) {
  chart = flamegraph()
    ...
    .minFrameSize(1)              // TODO: trap
    // set our custom handlers
    .setColorMapper(cpuColorMapper)
    .onClick(onClick)
    .tooltip(getTooltip());
